...
interface RecomendacionCopiloto {
  agenteNombre: string;
...
function CopilotoTab({
  ...
  setTab: (t: TabPanelId) => void;
}) {
  const [cargando, setCargando] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [rec, setRec] = useState<RecomendacionCopiloto | null>(null);
  ...
  useEffect(() => {
    ...
    fetch('/api/ai/copilot', {
      ...
      .catch((err) => {
        if (active) {
          setError(err.message || 'Error al invocar al Copiloto.');
          setCargando(false);
        }
      });
  ...
  function copiarPortapapeles() {
    if (!rec) return;
    navigator.clipboard.writeText(rec.borradorRespuestaInterna);
    setCopiado(true);
    setTimeout(() => setCopiado(false), 2000);
  }

  function adoptarBorrador() {
    if (!rec) return;
    setRespuesta(rec.borradorRespuestaInterna);
    setTab('respuesta');
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-3">
        ...
        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest animate-pulse">
          Invocando Copiloto de la Secretaría…
        </p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="rounded-xl border border-rose-500/20 bg-rose-500/10 p-4 text-xs text-rose-300">
        <p className="font-bold">Error del Copiloto IA:</p>
  ...
  const esCritica = rec.priorizacionSugerida === 'CRITICA';
  const esPreven = rec.priorizacionSugerida === 'PREVENTIVA';
  ...
  return (
    <div className="space-y-5 text-slate-300">
      ...
      <div className="flex items-center justify-between border-b border-white/[0.07] pb-2">
        ...
        <span
          className={`px-2 py-0.5 rounded-full text-[9px] font-black tracking-widest border uppercase ${
            esCritica
        ...
        >
          {rec.priorizacionSugerida}
        </span>
      </div>

      {/* Motivo Prioridad */}
      <div className="bg-white/[0.01] border border-white/[0.04] p-3 rounded-xl space-y-1">
        <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500">Priorización Sugerida</p>
        <p className="text-xs text-slate-300 leading-normal">{rec.motivoPrioridad}</p>
      </div>

      {/* Sugerencias Operativas */}
      <div className="space-y-2.5">
        <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500">Acciones Recomendadas</p>
        <div className="space-y-2">
          {rec.sugerenciasOperativas.map((sug, idx) => (
            <div key={idx} className="flex gap-2.5 items-start p-2.5 rounded-xl border border-white/[0.02] bg-white/[0.01] hover:bg-white/[0.02] transition-colors">
              <span className="text-emerald-400 font-bold shrink-0 text-xs">✓</span>
              <span className="text-xs text-slate-300 leading-normal">{sug}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-2">
        <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500">Borrador de Sugerencia</p>
        <div className="relative">
          <textarea
            readOnly
            value={rec.borradorRespuestaInterna}
            rows={8}
            className="w-full text-[11px] font-mono leading-relaxed bg-black/40 border border-white/[0.06] rounded-xl p-3 text-slate-300 resize-none focus:outline-none"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-1">
        <button
          ...
          className="w-full py-2.5 rounded-xl border border-white/10 text-slate-300 text-xs font-bold hover:bg-white/[0.05] active:scale-[0.98] transition-all duration-150 focus-visible:outline-none"
        >
          {copiado ? '¡Copiado!' : 'Copiar Texto'}
        </button>
        <button
          type="button"
          onClick={adoptarBorrador}
          className="w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 active:scale-[0.98] text-white text-xs font-bold transition-all duration-150 focus-visible:outline-none"
        >
          Adoptar Borrador
        </button>
      </div>
