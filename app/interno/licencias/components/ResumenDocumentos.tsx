  ...
  return (
    ...
    >
      ...
      <ul className="flex flex-col divide-y" style={{ borderColor: 'var(--color-border)' }}>
        {recientes.map((d) => (
          <li key={d.id} className="flex min-w-0 flex-col gap-1 py-2.5 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
            <div className="min-w-0">
              <p className="break-words text-sm font-bold" style={{ color: 'var(--text-primary)', overflowWrap: 'anywhere' }}>
                {d.nombre}
              </p>
              <p className="mt-0.5 break-words text-xs" style={{ color: 'var(--text-secondary)', overflowWrap: 'anywhere' }}>
