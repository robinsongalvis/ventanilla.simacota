...
describe('evaluarCondicion — árbol de expresión booleana', () => {
  ...
  it('IGUAL', () => {
    expect(evaluarCondicion({ operador: 'IGUAL', clave: 'esApoderado', valor: false }, contexto)).toBe(true);
    expect(evaluarCondicion({ operador: 'IGUAL', clave: 'esApoderado', valor: true }, contexto)).toBe(false);
  });

  it('DISTINTO', () => {
    expect(evaluarCondicion({ operador: 'DISTINTO', clave: 'esApoderado', valor: true }, contexto)).toBe(true);
  });
  ...
  it('Y — todas deben cumplirse', () => {
    ...
    expect(evaluarCondicion(c, contexto)).toBe(true);
  });
...
describe('requisitoAplica', () => {
  it('OBLIGATORIO siempre aplica', () => {
    expect(requisitoAplica({ id: 'x', nombre: 'x', tipo: 'OBLIGATORIO' }, {})).toBe(true);
  });

  it('OPCIONAL nunca aplica (no bloquea)', () => {
    expect(requisitoAplica({ id: 'x', nombre: 'x', tipo: 'OPCIONAL' }, {})).toBe(false);
  });
  ...
  it('CONDICIONAL depende de su regla', () => {
    ...
    expect(requisitoAplica(req, { esApoderado: true })).toBe(true);
    expect(requisitoAplica(req, { esApoderado: false })).toBe(false);
  });
});
