...
| **RS-1** | `atLocalNoon` (`lib/tiempos-radicado.ts`) extraía el día civil con getters de TZ del ENTORNO (proceso) en vez de anclar a `America/Bogota` — en un servidor corriendo en UTC (Vercel, por defecto), cualquier instante ≥ ~19:00 hora Bogotá caía en el día civil SIGUIENTE, desplazando ±1 día los vencimientos legales (deuda #15, ADR-0026 §A2; RS-1 del ultrareview de #146) | MEDIA (**→ ALTA** al cablear el reloj a endpoints/cron) | **MITIGADO 2026-08-07** — `atLocalNoon` ahora extrae año/mes/día con `Intl.DateTimeFormat` anclado a `America/Bogota` (mismo patrón de `lib/fecha-colombia.ts`), independiente de la TZ del proceso. Dictamen jurídico gobierno-digital: **CUMPLE**, defecto de implementación, el fix restituye conformidad (`docs/planes/DICTAMEN_TZ_DIA_CIVIL.md`, 2026-08-07) — hora legal colombiana UTC−5 (Decreto 2707/1982), custodio INM (D.L. 4175/2011 art. 6.14). Control de regresión: `__tests__/atlocal-noon-tz-colombia.test.ts`. **Residual transicional declarado**: vencimientos de radicados EN TRÁMITE calculados antes del fix con el defecto quedan almacenados con el sesgo (+1 día, unidireccional) hasta ejecutar el barrido one-off (`scripts/laboratorio/barrido-vencimientos-tz.mjs`, modo `--dimensionar` entregado; modo `--corregir` **pendiente de autorización explícita del propietario**, protocolo autorización→ejecución — nunca acorta plazos ya comunicados al ciudadano, CPACA art. 3.4). Radicados cerrados NO se tocan (Ley 594/2000 art. 19). | ADR-0026 §A2 (deuda #15); `docs/planes/DICTAMEN_TZ_DIA_CIVIL.md`; `lib/tiempos-radicado.ts`; `__tests__/atlocal-noon-tz-colombia.test.ts`; `scripts/laboratorio/barrido-vencimientos-tz.mjs` |
