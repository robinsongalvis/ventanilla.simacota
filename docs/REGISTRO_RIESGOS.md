...
|----|----------|------|----------------|--------|---------------|--------------|
| **H1** | Prórroga sin límite de unicidad **ni** tope del doble del término — permite correr el vencimiento del derecho de petición indefinidamente | **ALTA** | Ley 1755/2015 art. 14 y parágrafo; art. 23 C.P. | **EN DECISIÓN** — excepción al congelamiento (Regla Suprema) pendiente del propietario | dev-backend + arquitecto (cómputo del tope, Nivel 3 → ADR) | `docs/laboratorio/CONCEPTO_NORMATIVO_FASE2.md`; `e2e/09-prorroga-con-notificacion.spec.ts`; `app/api/radicados/[radicadoId]/prorroga/route.ts` |
| **H2** | Identidad reservada visible en vistas internas (detalle, bandeja) pese a estar enmascarada en mostrador/consulta/MIPG | MEDIA-ALTA (ALTA si denunciante) | Ley 1581/2012 art. 4 f/g/h | ABIERTO (backlog) | dev-frontend (reutilizar `identidadProtegida`) | `docs/laboratorio/CONCEPTO_NORMATIVO_FASE2.md`; `e2e/07-identidad-reservada.spec.ts`; `app/interno/dashboard/page.tsx` ~2493-2500, ~4290 |
