...
| **Gate de auditoría gobernado (`npm run audit:gate`)** | Vulnerabilidades de dependencias high/critical, con allowlist auditable y caducidad (rompe el hard-deadlock por transitivas sin fix upstream sin debilitar el gate) | `ci.yml` job `validate` · paso `Security Scan` → [`scripts/ci/audit-gate.mjs`](../scripts/ci/audit-gate.mjs) | [ADR-0028](adr/0028-gate-de-auditoria-gobernado.md); probado por mutación (`__tests__/audit-gate.test.ts`): advisory nueva no allowlistada → falla; entrada de allowlist vencida → falla |
...
| [0023](adr/0023-blueprint-arquitectonico.md) | Blueprint Arquitectónico: especificación formal previa a toda implementación | aceptado (2026-07-14) | Ninguna capacidad "lista" solo por su ficha; exige Blueprint completo (23 secciones: arq. funcional/lógica, límites de dominio, entidades/agregados, eventos, reglas, flujos, actores, permisos, APIs, integraciones, datos, reutilización, componentes nuevos justificados, impactos SIMI/seguridad/auditoría/rendimiento/mantenibilidad, riesgos, migración/pruebas/despliegue) + análisis crítico obligatorio (7 preguntas, incl. proyección a 5 años) con **bucle de re-revisión automático** si se puede simplificar/consolidar más. Definition of Ready = Blueprint + Cuatro Preguntas + Valor Neto + análisis crítico. Plantilla en [blueprints/_PLANTILLA_BLUEPRINT.md](blueprints/_PLANTILLA_BLUEPRINT.md) |
