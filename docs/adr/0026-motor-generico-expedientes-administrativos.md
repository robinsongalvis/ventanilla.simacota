...
| 7 | Guard D9 `verificarAvanceCounter`: **cableado a `confirmarConsecutivosLegales`** (Fase 1) — todo avance de `counters` por la vía transaccional estándar pasa por el guard (monotonicidad + prohibición de `RECONSTRUIDO`) **antes** del `tx.set`, con `ultimoActual` capturado en la lectura (no derivado del propuesto). Cubre la serie `expedientes` por diseño (precondición #4). **Restan, desacoplados:** (a) *narrowing* del wildcard `counters` en `firestore.rules` — atado al **cutover** de la pieza angular (CR-1): con `USA_RADICACION_INTERNA_SERVER=false` el cliente aún escribe `counters` y cerrar la regla rompería la radicación en prod; (b) extender el detector de fantasmas a `expedientes` — requiere la colección (Fase 2). | MEDIA | **Fase 1** (cableado server-side ✅) · reglas con cutover · detector con Fase 2 |
