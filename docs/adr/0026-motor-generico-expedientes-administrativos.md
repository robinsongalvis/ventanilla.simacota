...
| 12 | `COLECCION_POR_SERIE` del detector de fantasmas no incluye `expedientes` | BAJA | **Vía superada 7-ago-2026**: extender `COLECCION_POR_SERIE`/`huecosDe`/`consecutivoDeId` es estructuralmente imposible para este id — el número de expediente (`{dane}-{curaduria}-{AA}-{CCCC}`) lleva año de 2 dígitos en el PENÚLTIMO segmento, no el último; reutilizar esas funciones sin cambiarlas produciría 19 huecos falsos (`perteneceAlAnio` nunca matchea `-26-`). En su lugar, `app/api/cron/auditoria-consecutivos/route.ts` tiene una rama propia (`auditarCounterExpedientes`) que reporta el ESTADO del counter (`SIN_ABRIR`/`PARCIAL`/`CORRUPTO`) sin auditoría de continuidad contra documentos — no hay colección que barrer todavía. **Cierre definitivo** (sigue en Fase 1): auditoría de continuidad POR CAMPO (no por id parseado) cuando firestore-datos fije el esquema real de la colección de expedientes |
...
| 15 | `atLocalNoon` (`tiempos-radicado.ts`) extrae el día calendario con getters de TZ del entorno en vez de anclar a `America/Bogota` → desplazamiento de ±1 día en el límite de subsanación (RS-1 del ultrareview). **Pre-existente** (afecta también a BM-B33), NO introducido por el motor | ~~MEDIA (→ ALTA al cablear el reloj)~~ **CERRADA 7-ago-2026** | `atLocalNoon` ahora ancla con `Intl.DateTimeFormat` a `America/Bogota` (patrón de `lib/fecha-colombia.ts`), independiente de la TZ del proceso. Validación jurídica: **CUMPLE** — defecto de implementación, no cambio de criterio (`docs/planes/DICTAMEN_TZ_DIA_CIVIL.md`, gobierno-digital, 2026-08-07). Test del anclaje: `__tests__/atlocal-noon-tz-colombia.test.ts`. Residual transicional (RS-1 en `docs/REGISTRO_RIESGOS.md`): vencimientos ya calculados con el defecto en radicados EN TRÁMITE requieren el barrido one-off `scripts/laboratorio/barrido-vencimientos-tz.mjs` (modo `--corregir` pendiente de autorización del propietario) |
| 16 | Brechas de resistencia a mutación en la suite (día exacto en la ruta string-ISO del reloj, `festivosExtra` sin ejercitar, `clavesFaltantes` con condición compuesta multi-clave). El operador `NO` en dirección `CUMPLE→NO_CUMPLE` (COB-1) **ya se cerró en #146** | BAJA | **Parcialmente cerrada 7-ago-2026**: el sub-ítem "día exacto en la ruta string-ISO del reloj" queda cubierto por `__tests__/atlocal-noon-tz-colombia.test.ts` (compara instante ISO-con-offset vs. su equivalente UTC y exige el día EXACTO, no solo "cae en día hábil" — mismo patrón que cerró el sub-ítem análogo del motor en RS-3). Siguen abiertos para **Fase 1**: `festivosExtra` sin ejercitar y `clavesFaltantes` con condición compuesta multi-clave (endurecimiento de tests por QA) |
